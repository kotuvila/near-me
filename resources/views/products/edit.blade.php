                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="row form-group {{ $errors->has('id') ? 'has-error' : '' }}">
                            <div class="col-md-3">
                                <lable>ID<span style="color: red">*</span>:</lable><br>
                            </div>
                            <div class="col-md-9">
                                <input type="text" value="{{ $item->id }}" readonly name="id" id="ID"
                                    class="form-control form-control-sm"
                                    value="{{ old('id', isset($item) ? $item->id : '') }}" />
                                @if ($errors->has('id'))
                        ...
                        <div class="row form-group {{ $errors->has('code') ? 'has-error' : '' }}">
                            <div class="col-lg-3">
                                <lable>Item Code<span style="color: red">*</span>:</lable>
                            </div>
                            <div class="col-lg-9">
                        ...
                        <div class="row form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                            ...
                            <div class="col-lg-9">
                                <textarea type="text" name="description" class="form-control form-control-sm"
                                    placeholder="Enter  Description Here">{{ old('description', isset($item) ? $item->description : '') }}</textarea>
                        ...
                        <div class="row form-group {{ $errors->has('supplier') ? 'has-error' : '' }}">
                            <div class="col-lg-3">
                                <lable> Supplier<span style="color: red">*</span>:</lable>
                            </div>
                                ...
                                <select class="form-control select2" name="suppliers[]" id="supplier" multiple="multiple">
                                    @foreach ($suppliers as $id => $supplier)
                                        <option value="{{ $supplier->id }}"
                                            {{ in_array($id, old('suppliers', [])) || (isset($item) && $item->supplier->contains($id)) ? 'selected' : '' }}>
                                            {{ $supplier->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row form-group {{ $errors->has('brand') ? 'has-error' : '' }}">
                            ...
                            <div class="col-lg-9">
                                <select class="form-control form-control-sm" name="brand_id" id="brand_id"
                                    value="{{ old('brand', isset($item) ? $item->category : '') }}">
                                    <option hidden disabled>Select Brand</option>
                                    @foreach ($brands as $id => $brand)
                                        <option value="{{ $brand->id }}"
                                            {{ isset($brand) && $brand->id == $item->brand_id ? 'selected' : '' }}>
                                            {{ $brand->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-control form-control-sm" name="category_id" id="category_id"
                                    ...
                                    @foreach ($categories as $id => $category)
                                        <option value="{{ $category->id }}"
                                            {{ isset($category) && $category->id == $item->category_id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row form-group {{ $errors->has('subcategory') ? 'has-error' : '' }}">
                            ...
                            <div class="col-lg-9">
                                <select class="form-control form-control-sm" name="subcategory_id" id="subcategory_id"
                                    value="{{ old('subcategory', isset($item) ? $item->subcategory : '') }}">
                                    <option hidden disabled>Select Subcategory</option>
                                    @foreach ($subcategories as $id => $subcategory)
                                        <option value="{{ $subcategory->id }}"
                                            {{ isset($subcategory) && $subcategory->id == $item->subcategory_id ? 'selected' : '' }}>
                                            {{ $subcategory->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row form-group {{ $errors->has('sale_price') ? 'has-error' : '' }}">
                            <div class="col-lg-3">
                                <lable>Sale Price<span style="color: red">*</span>:</lable>
                            </div>
                            <div class="col-lg-9">
                                <input type="text" name="sale_price" class="form-control form-control-sm"
                                    placeholder="Enter Sale  Price Here"
                                    value="{{ old('sale_price', isset($item) ? $item->sale_price : '') }}">
                        ...
                        <div class="row form-group {{ $errors->has('amount') ? 'has-error' : '' }}">
                            <div class="col-lg-3">
                                <lable>Amount<span style="color: red">*</span>:</lable>
                            </div>
                            <div class="col-lg-4">
                            ...
                            <div class="col-lg-4">
                                <select name="amount_type" class="form-control form-control-sm"
                                    placeholder="Enter Amount Type Here"
                                    value="{{ old('amount_type', isset($item) ? $item->amount_type : '') }}">
                                    >
                                    <option hidden disabled>Select Amount Type</option>
                                    <option value="mg"
                                        {{ old('amount_type', $item->amount_type == 'mg' ? 'selected' : '') }}>mg
                                    </option>
                                    <option value="ml"
                                        {{ old('amount_type', $item->amount_type == 'ml' ? 'selected' : '') }}>ml
                                    </option>
                                </select>
                        ...
                        <div class="row form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <div class="col-lg-9">
                                <select name="status" class="form-control form-control-sm"
                                    value="{{ old('status', isset($item) ? $item->status : '') }}">
